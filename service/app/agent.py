...
def build_agent(wiki_root: Path, model_id: str | None = None) -> Agent[WikiDeps, str]:
    ...
    Args:
        ...
        model_id: Optional Override des Modells. Default = ENV `MODEL_ID`
            oder `google-gla:gemini-flash-latest` als Standard.
            Wir nehmen den `-latest`-Alias bewusst: feste Preview-IDs werden
            regelmäßig abgekündigt (gemini-3-pro-preview → 404 ab 03/2026),
            der Alias zieht automatisch aufs neueste Flash nach. MODEL_ID
            per ENV überschreibbar, falls ein fest gepinntes Modell nötig ist.
    ...
    """
    model = model_id or os.environ.get("MODEL_ID", "google-gla:gemini-flash-latest")
